...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    peer chaincode instantiate -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C mychannel -n mycc -v 1.0 -c '{"Args":["init"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer')" >&log.txt
    res=$?
    set +x
  else
    set -x
...
chaincodeQuery() {
  ...
  do
    ...
    peer chaincode query -C mychannel -n mycc -c '{"Args":["queryLand","'$LANDNAME'"]}' >&log.txt
    res=$?
    set +x
...
chaincodeInvokeChangeOwner() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n mycc -c '{"Args":["changeLandOwner","'$LANDNAME'","'$OWNERNAME'"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
  ...
  #echo invokechangelandowner >> scripts/abc.txt
...
chaincodeInvokeForkLand() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    #peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS -c '{"Args":["invoke","a","b","10"]}' >&log.txt
    peer chaincode invoke -o orderer.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n mycc -c '{"Args":["forkLand","'$LANDNAME'","'$CHILD1LANDNAME'","'$OWNER1NAME'","'$EXTENT1'","'$CHILD2LANDNAME'","'$OWNER2NAME'","'$EXTENT2'","'$DIVISIONDIR'","'$XDIVCENTRE'","'$YDIVCENTRE'"]}' >&log.txt
    res=$?
    set +x
  else
    set -x
  ...
  #echo invokeforkland >> scripts/abc.txt
  cat log.txt>>scripts/abc.txt
  verifyResult $res "Invoke execution on $PEERS failed "
  echo "===================== Invoke transaction successful on $PEERS on channel '$CHANNEL_NAME' ===================== "
  echo
...
chaincodeStartTime() {
  ...
  echo $starttime>>scripts/abc.txt
}
